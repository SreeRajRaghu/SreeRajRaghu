...
//            self.ks_fetch_data();
        ...
        events: {
            ...
            'click .custom-date-range-fltr-trigger': '_onLoadCustomModal',
            'click .custom-date-range-fltr-close-button': '_onLoadCustomModalClose',
            'click .custom-date-range-fltr-apply-button': '_onCustomApplyDateFilter',
                            ...
                            if(item_to_update){
                                ...
                                if(update_notification_ids.length>0){
                                    self.call('notification', 'notify', {
                                        message: "" + item_to_update.length + " Dashboard items has been updated.",
                                        type: 'info',
                                    });
                                }
...
//                                ));
        ...
        _ksRenderDashboardTile: function(tile) {
            ...
            if(tile.ks_data_formatting == 'exact'){
                data_count = tile.ks_record_count;
            }else{
                if(tile.ks_data_formatting == 'indian'){
                    data_count = self.ksNumIndianFormatter(tile.ks_record_count, 1);
                }else{
                    data_count = self.ksNumFormatter(tile.ks_record_count, 1);
                }
            }
                            ...
                            callback: function(value, index, values) {
                                ...
                                if (ks_selection === 'monetary') {
                                    ...
                                    if(item.ks_data_formatting == 'exact'){
                                        ...
                                        if (item.ks_data_formatting == 'indian'){
        ...
        ksFetchChartItem: function(id) {
            ...
            return self._rpc({
                ...
                if (item_data.ks_list_view_data) {
                    ...
                    if (JSON.parse(item_data['ks_list_view_data']).data_rows.length) {
...
//                            additional_context: action.context,
        ...
        ksUpdateDashboardItem: function(ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['ks_dashboard_item_type'] == 'ks_list_view') {
                    ...
                    if (item_data.ks_record_data_limit == 15 || item_data.ks_record_count == 15) {
        ...
        _renderListView: function(item, grid) {
            ...
            else if (item.ks_record_data_limit == 15 || item.ks_record_count == 15) {
                $ks_gridstack_container.find('.ks_load_next').addClass('ks_event_offer_list');
            }
        ...
        ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
            ...
            if(field.ks_data_formatting == 'exact'){
               ...
               item_info['count_tooltip'] = count
                ...
                if(field.ks_data_formatting == 'indian'){
        ...
        ksOnListItemInfoClick: function(e) {
            ...
            var item_data = self.ks_dashboard_data.ks_item_data[item_id];
            var domain = []
            if (e.currentTarget.parentElement.parentElement.dataset &&
                e.currentTarget.parentElement.parentElement.dataset.domain){
                domain = JSON.parse(e.currentTarget.parentElement.parentElement.dataset.domain);
                if(item_data.ks_domain && item_data.ks_domain.length > 0){
                    for (var i = 0; i < item_data.ks_domain.length; i++)
                    domain.push(item_data.ks_domain[i]);
                }
            }
            if(domain.length === 0){
                domain = item_data.ks_domain || [];
        ...
        _onLoadCustomModal: function(e){
            $('.custom-date-range-fltr-modal').addClass('custom-date-range-fltr-show-modal')
        },
        _onLoadCustomModalClose: function(e){
            $('.custom-date-range-fltr-modal').removeClass('custom-date-range-fltr-show-modal')
        },

         _onCustomApplyDateFilter: function(e) {
            var self = this;
            var start_date = $('.customStart-Date-filter').val();
            var end_date = $('.customEnd-Date-filter').val();
            if (start_date === "Invalid date") {
                alert("Invalid Date is given in Start Date.")
            } else if (end_date === "Invalid date") {
                alert("Invalid Date is given in End Date.")
                ...
                if (start_date && end_date) {
                    ...
                    if (moment(start_date, 'YYYY-MM-DD H:m:s') <= moment(end_date, 'YYYY-MM-DD H:m:s')) {
                        if (start_date === "Invalid date" || end_date === "Invalid date"){
                            alert(_t("Invalid Date"));
                        }else{
                            self.ksDateFilterSelection = 'l_custom';
                            self.ksDateFilterStartDate = start_date;
                            self.ksDateFilterEndDate = end_date;

                            self.ks_fetch_items_data().then(function(result){
                                self.ksUpdateDashboardItem(Object.keys(self.ks_dashboard_data.ks_item_data));
                                self._onLoadCustomModalClose()
                            });
                       }
        ...
        ksChartExportXlsCsv: function(e) {
            ...
            var params = self.ksGetParamsForItemFetch(parseInt(chart_id));
            var data = {
                "header": name,
                "chart_data": this.ks_dashboard_data.ks_item_data[chart_id].ks_list_view_data,
                "ks_item_id": chart_id,
                "ks_export_boolean": this.ks_dashboard_data.ks_item_data[chart_id].ks_export_all_records,
                "context": context,
                "params":params,
            }
            }else{
