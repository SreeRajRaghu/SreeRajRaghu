...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_chart_relation_groupby = fields.Many2one('ir.model.fields',
                                                domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
                                                       ...
                                                       "('ttype','!=','many2many'), ('ttype','!=','one2many'),('ttype','!=','html')]",
                                                string="Group By")
    ...
    ks_export_all_records = fields.Boolean(string="Export All Records", default=False, help="when click on boolean button, all the records will be downloaded which are present in entire list")
    ...
    ks_data_format = fields.Selection([
        ...
        ('exact', 'Exact Value')
    ], string='Number System', default='global')
    ...
    def ks_date_target(self):
        for rec in self:
            ...
            if rec.ks_chart_relation_sub_groupby.ttype == 'date' and rec.ks_chart_date_sub_groupby in ('minute', 'hour'):
                raise ValidationError(_('Groupby field: {} cannot be aggregated by {}').format(
    ...
    def _ksGetListViewData(self,domain=[]):
        ...
        if rec.ks_list_view_type and rec.ks_dashboard_item_type and rec.ks_dashboard_item_type == 'ks_list_view' \
            ...
            ks_chart_domain = self.ks_convert_into_proper_domain(self.ks_domain, self,domain)
    ...
    def get_list_view_record(self, orderid,sort_order, ks_chart_domain, offset=0, limit=15, ks_export_all=False):
        ...
        if ks_export_all:
            limit = 0
            offset =0
            if self.ks_record_data_limit > 0:
                limit = self.ks_record_data_limit
            ...
            if self.ks_chart_groupby_type == 'relational_type':
                ...
                ks_list_view_records = self.env[self.ks_model_name]. \
                    read_group(ks_chart_domain, ks_list_fields, [self.ks_chart_relation_groupby.name],
                               orderby=orderby, limit=limit, offset=offset, lazy=False)
    ...
    @api.model
    def ks_fetch_list_view_data(self,rec, ks_chart_domain, limit=15, offset=0, ks_export_all=False):
        ks_list_view_data = {'label': [], 'fields': [],  'fields_type': [],
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                ...
                elif ks_list_view_field_type[counter] == "selection" and res[field_rec]:
    ...
    def ks_get_next_offset(self, ks_item_id, offset, item_domain=[]):
        ...
        if self.ks_list_view_type == 'grouped':
            ...
            ks_list_view_data = self.get_list_view_record(orderby, sort_order, ks_list_domain, offset=int(ks_offset))
