...
class ExportAttendance2(models.TransientModel):
    _name = 'wizard.export.attendance.calendar'
    _description = 'Report Attendance with Calendar'
    ...
    def get_leave_data(self, start, end, emp_ids):
        ...
        l.request_date_from, l.request_date_to, DATE(l.date_resume), l.id
        FROM hr_leave AS l
    ...
    def get_att_data(self):
        ...
        SELECT
            ...
            a.check_in, a.check_out, a.worked_hours, a.is_missed
    ...
    def export_attendance_calendar(self):
        self.ensure_one()
        Employee = self.env['hr.employee']
        ...
        def get_interval_by_dt(intervals, dt):
            return list(filter(lambda i: i[0].date() == dt.date(), intervals))
        ...
        for emp_id, data in attendance_data.items():
            ...
            # result = calendar._get_day_total(timezone_datetime(start_date, tz_name=tz_name), timezone_datetime(end_date, tz_name=tz_name))
            intervals = Attendance.get_resource_attendance(start_date, end_date, calendar)
            ...
            worksheet.write(row, col, _('Current Calendar :: In/Out Report :: From %s To %s') % (user_from_date, user_to_date), style_title)
            ...
            total = tot_late_in = tot_early_out = tot_ot = tot_ut = 0
            ...
            for att in sorted_attendances:
                ...
                actual_in_list = get_interval_by_dt(work_intervals, check_in)

                early_out_str = late_in_str = late_in = early_out = ''
                diff_hours = 0
                ...
                if actual_in_list:
                    ...
                    actual_hours = sum(
                        ...
                        for start, stop, meta in actual_in_list
                    )
                    worked_hours = att.get('worked_hours') or 0
                    if worked_hours:
                        diff_hours = worked_hours - actual_hours

                    actual_in = actual_in_list[0][0]
                    actual_out = actual_in_list[-1][1]
                    late_in = check_in - actual_in
                    # a.check_in - a.actual_in
                    if late_in and worked_hours:
                    ...
                    early_out = actual_out - check_out
                    if early_out and worked_hours:
        ...
        file_name = "Attendance-Calendar.xlsx"
        return {
            'target': 'new',
