...
RESOURCE_FIELDS_IN_MODEL = {
    'medical.order': 'resource_id',
    'account.move': 'resource_id',
    'medical.lab.test': 'resource_id'
}
...
class Dashboard(models.Model):
    ...
    def has_active(self, model, field_name='active'):
        for field in model.field_id:
            if field.name == field_name and field.store:
                return True
        return False
    ...
    def _compute_field_list(self):
        ...
        for list in lists:
            if list.display:
                ...
                alias = list.model_id.model.replace('.', '_')
                if not action:
                ...
                if self.has_active(list.model_id, 'company_id'):
    ...
    def action_setting(self):
        action = self.env.ref('medical_dashboard.action_dashboard_config').read()[0]
        setting = self.env['dashboard.settings'].search([], limit=1, order='id desc').id
        action['views'] = [(self.env.ref('medical_dashboard.dashboard_config_settings').id, 'form')]
