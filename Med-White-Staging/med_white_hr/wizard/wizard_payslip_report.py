...
class WizardPayslipReport(models.TransientModel):
    _inherit = 'wizard.payslip.report'
    ...
    def print_bank_excel(self):
        ...
        payslips_heading = [
           'Employee Number', 'Employee Name', 'IBAN', 'Payment Amount', 'Bank Code', 'Civil ID','Company'
        ]
    ...
    def print_excel(self):
        ...
        payslips_heading = [
            'S/N', 'Slip Ref.', 'Employee ID', 'English Name', 'Arabic Name', 'Company', 'branch', 'Analytic Account', 'Nationality', 'Civil ID', 'Department', 'Job Title',
        ...
        for slip in payslips:
            ...
            if not emp_data.get(emp.id):
                ...
                emp_data.update({
                    emp.id: {
                        ...
                        'branch': slip.contract_id.branch_id.name or '',
                        'analtyic': slip.contract_id.analytic_account_id.name or '',
            ...
            col += 1
            worksheet.write(row, col, emp_data[emp.id]['company_name'] or '')
            col += 1
            worksheet.write(row, col, emp_data[emp.id]['branch'] or '')
            col += 1
            worksheet.write(row, col, emp_data[emp.id]['analtyic'] or '')
            col += 1
