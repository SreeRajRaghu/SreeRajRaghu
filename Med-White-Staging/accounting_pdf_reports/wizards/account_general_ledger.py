...
class AccountReportGeneralLedger(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        data['form'].get('used_context', {}).update({
            'general_account_ids': self.account_ids,
            'general_partner_ids': self.partner_ids.ids,
            'general_analytic_account_ids': self.analytic_account_ids.ids,
        })
        records = self.env[data['model']].browse(data.get('ids', []))
        return self.env.ref('accounting_pdf_reports.action_report_general_ledger').with_context(landscape=True,company_id=self.company_id.id).report_action(records, data=data)
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        partner_ids = self.env.context.get('general_partner_ids', [])
        if partner_ids:
            partner_condition = {
                'sql': " AND p.id IN %s ",
        ...
        analytic_account_ids = self.env.context.get('general_analytic_account_ids', [])
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode, 0.0 AS amount_currency, '' AS lref, 'Initial Balance' AS lname, '' AS involdseq, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            m.name AS move_name, c.symbol AS currencyrency_code, p.name AS partner_name\
            FROM account_move_line l\
            ...
            LEFT JOIN account_analytic_account aa ON (l.analytic_account_id=aa.id) \
            WHERE l.account_id IN %s ''' + partner_condition['sql'] + analytic_condition['sql'] + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, l.currency_id, l.amount_currency, l.ref, l.name, m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
