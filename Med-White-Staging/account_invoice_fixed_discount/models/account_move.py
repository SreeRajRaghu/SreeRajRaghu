...
class AccountMoveLine(models.Model):
    ...
    def _get_price_total_and_subtotal_model(
        ...
        if self.discount_fixed != 0:
            if price_unit:
                discount = ((((self.discount_fixed) / price_unit) * 100) / self.quantity) or 0.00
            else:
                discount = 0.0
    ...
    def _get_fields_onchange_balance_model(
        ...
        if self.discount_fixed != 0:
            discount = ((((self.discount_fixed) / self.price_unit) * 100) / self.quantity) or 0.00
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get("discount_fixed"):
                ...
                if fixed_discount:
                    fixed_discount = fixed_discount / vals.get("quantity")
