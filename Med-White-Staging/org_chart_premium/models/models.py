...
class OrgChartDepartment(models.Model):
    ...
    def get_employee_data(self):
        ...
        child_company = self.env['res.company'].sudo().search([('parent_id', '=', company.id)])
        if child_company[:1] and child_company[:1].id != company.id:
            for r in child_company:
                child_company_data = {
                    'id': -1,
                    'name': r.name,
                    'title': r.country_id.name,
                    'children': [],
                    'office': "<img class='avatar company' src='/logo.png' />",
                }
                ...
                for employee in employees:
                    child_company_data['children'].append(self.get_children(employee, r, 'middle-level'))
                data['children'].append(child_company_data)
        else:
            employees = self.env['hr.employee'].search([
                ('parent_id', '=', False),
            ...
            for employee in employees:
                data['children'].append(self.get_children(employee, company, 'middle-level'))

        return {'values': data}

    @api.model
    def get_children(self, emp, company, style=False):
        data = []
        emp_data = {'id': emp.id, 'name': emp.name, 'title': self._get_position(emp),
        ...
        for child in childrens:
            ...
            if not sub_child:
                data.append(
                    {'id': child.id, 'name': child.name, 'title': self._get_position(child), 'className': next_style,
                     'office': self._get_image(child, style)})
            else:
                data.append(self.get_children(child, company, next_style))
    ...
    def get_emp_form_id(self):
        ...
        return {
            ...
            'vertical_level': int(
                org_chart_config.vertical_level) if org_chart_config.vertical_level != 'none' else False,
        }
